...
@ColumnFamily("Coupon")
@Indexes({ 
	@Index(fields = "brand", 	order = "expiring desc,brand asc"),
	@Index(fields = "expiring", 	order = "expiring desc"),
	})
@NamedQueries({ 
	@NamedQuery(name = "likeBrandByExpiring", 		query = "select c from Coupon c where c.brand >= :brandStart and c.brand <= :brandEnd order by c.expiring desc,c.brand asc")
	})
